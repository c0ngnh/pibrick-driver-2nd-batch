 ...
 *
 * Pi CM5 / kernel 6.18 port: enable-disable callbacks, sysfs, shutdown,
 * and LPM-safe power sequencing (matches panel-pibrick.9203.c).
 */
...
/* Use 9203 DSI porch timings: legacy 9202 values (HFP 72 / HSYNC 112 / HBP 184)
 * run ~25% faster pixel clock and scroll-tear on Pi 5 GNOME; 9203 @ 60 Hz does not. */
...
static int visionox_vtdr6110_on(struct visionox_vtdr6110 *ctx)
{
	...
	/* TE on after bank init (9203 order); TE GPIO is dummy on PocketCM5 but
	 * late 0x35 may reduce scroll tearing on Pi 5 DSI video mode. */
...
static void visionox_vtdr6110_read_id(struct visionox_vtdr6110 *ctx)
{
	...
	if (r1 < 0 && r2 < 0 && r3 < 0 && rddb < 0 && rpwr < 0) {
		dev_info(dev,
			 ...
			 "Identify by behavior instead (PANEL=9203 A/B test).\n");
		return;
	}
	...
	dev_info(dev,
		 "DDIC ID: this build sends the VTDR6110 (9202) init sequence; "
		 "a clean image here indicates a 9202 panel.\n");
}
...
MODULE_DESCRIPTION("piBrick 9202 AMOLED panel (1080x1240 @ 60 Hz) for Raspberry Pi 5 DSI1");
MODULE_LICENSE("GPL");
